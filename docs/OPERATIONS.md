...
> **`MEDIA_ROOT` в env сервиса обязателен** (2026-09-12). Раскладка `releases/<sha>/current`
> меняет cwd каждым деплоем; без переменной приложение писало бы в `<релиз>/media`, и сканы
> исчезали бы со следующим релизом. Значение — общий каталог вне релизов, тот же, что
> `BACKUP_MEDIA_ROOT` (бэкап `media/` смотрит туда же). Признак: `MEDIA_ROOT` в
> `/proc/<MainPID>/environ` сервиса и совпадение с `BACKUP_MEDIA_ROOT`.
...
`/api/health` ходит в БД (`SELECT 1`), поэтому зелёный ответ означает и живое приложение,
и живой Postgres; на отказе БД роут отдаёт 500 со `status: error`.

### Сторож прода (R5)

`scripts/health_watch.sh`, cron каждые 10 минут. До 2026-09-04 наблюдения не было вовсе:
единственной записью в cron был бэкап, и падение чего угодно никем не замечалось — так
воркер напоминаний и проработал с 30 августа из удалённого каталога релиза.

Проверяет четыре признака: `karman.service` активен, `karman-reminders.service` активен,
`/api/health` отдаёт 200 с телом `ok`, лог бэкапа писался за последние 36 часов. Пишет в
Telegram владельцу **только при смене состояния** (упало → письмо, поднялось → письмо):
сторож, срабатывающий каждые 10 минут, иначе превращается в спам, а спам перестают читать.

```bash
# прогнать вручную
set -a; . /etc/karman/karman.env; set +a; bash /home/valstan/karman/scripts/health_watch.sh
# журнал сторожа
tail -n 50 /home/valstan/backups/health-watch.log
# состояние и кэш получателя
cat ~/.karman-health/last-state; ls -l ~/.karman-health/chat-id
```

`chat_id` берётся из `telegram_link` и кэшируется на диск **на каждом прогоне**, а не при
отправке: при мёртвом Postgres разрешить получателя уже нельзя, а это ровно тот случай,
ради которого сторож заведён. Пустой `~/.karman-health/chat-id` в логе — предупреждение
отдельной строкой: сигнал уйти не сможет.
