...
### Выдача машинным путём — в две руки (D-061)

Выдача между комнатами без владельца делается **двумя токенами**: источник **предлагает**,
получатель **принимает**. У обоих — токен своей комнаты с правом записи (принять чужое имя
в своё окружение — операция класса записи, как и раздать свой ключ). Пока получатель не
принял, предложение **не попадает** в его `GET /api/secrets` и имя у него **не занимает**.

**Источник — предложить** (`note` обязателен: основание уходит в аудит обеих комнат):

```bash
curl -sf -X POST "$VAULT_URL/api/secrets/grants" \
  -H "Authorization: Bearer $SECRETS_TOKEN" -H "Content-Type: application/json" \
  --data-binary '{"key":"INGEST_GATEWAY_KEY","target_slug":"setka","alias":"KAZANSKAYA_INGEST_KEY","note":"D-061, мандат brain 2026-09-03"}'
# → 201 {"ok":true,"id":14,"state":"pending"}
```

`alias` необязателен — по умолчанию имя у получателя совпадает с `key`. Повторный `POST`
на уже принятое имя даёт `409`; предложить можно и ключ, которого у вас ещё нет.

**Получатель — посмотреть, что предложили, и принять по allowlist имён.** CI получателя
сам не узнает о предложении, поэтому шаг деплоя опрашивает входящие и принимает **только
имена из своего списка** — это и есть согласие цели, а не «принять всё, что прислали»:

```bash
ALLOW="KAZANSKAYA_INGEST_KEY VMALMYZHE_INGEST_KEY"   # имена, которые вы ждёте
curl -sf "$VAULT_URL/api/secrets/grants?pending=1" -H "Authorization: Bearer $SECRETS_TOKEN" \
  | jq -r '.received[] | "\(.id) \(.aliasKey)"' \
  | while read -r id name; do
      case " $ALLOW " in *" $name "*)
        curl -sf -X POST "$VAULT_URL/api/secrets/grants/$id/accept" \
          -H "Authorization: Bearer $SECRETS_TOKEN" >/dev/null && echo "accepted $name";;
      esac
    done
# accept → {"ok":true,"id":14,"state":"active"}; дальше ключ приходит в обычный GET /api/secrets
```

**Посмотреть и отозвать** — с любой стороны:

```bash
curl -sf "$VAULT_URL/api/secrets/grants" -H "Authorization: Bearer $SECRETS_TOKEN"
# → {"slug":"setka","issued":[{"id":12,"sourceKey":"VK_TOKEN","targetSlug":"dkmalmyzh","state":"active",...}],
#    "received":[{"id":14,"aliasKey":"KAZANSKAYA_INGEST_KEY","sourceSlug":"kazanskayamalmyzh","state":"pending",...}]}
...
Источник отзывает свою выдачу (или своё предложение), получатель — отказывается от чужого
имени в своём окружении; отозванное предложение принять уже нельзя.

- `state` у каждой выдачи: `pending` — предложено, ждёт получателя; `active` — действует;
  `revoked` — отозвано.
- В аудите принципал — `room:<ваш slug>`, а не владелец; чем предъявились (паспорт или токен) —
  в тексте строки. Строки: `grant_proposed` у обеих комнат при предложении, `grant_in` /
  `grant_out` при принятии, `grant_revoked` при отзыве, `grant_read` у источника на каждое чтение.
- Коды: `400` — тело не по схеме; `403` — токен только для чтения; `404` — чужая выдача или
  комната-получатель не найдена; `409` — уже принято/отозвано, имя занято своим ключом
  получателя или другой принятой выдачей.

Почему в две руки, а не одним `POST` источника: выдача пишет ИМЯ ключа в чужое пространство
имён, значение приезжает получателю в общий `GET /api/secrets`, а рецепты выше учат
раскладывать полученное в переменные окружения CI. Токен одной комнаты означал бы
подстановку произвольной переменной в чужую сборку, и платит за это не тот, кто её
совершает. Согласие получателя — не формальность, а та самая граница.
