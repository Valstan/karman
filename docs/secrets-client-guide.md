...
### Посмотреть и отозвать свои выдачи — токеном комнаты

Токеном своей комнаты с правом записи вы видите, что она выдала, и можете отозвать выдачу:
...
- В аудите принципал — `room:<ваш slug>`, а не владелец; чем предъявились (паспорт или токен) —
  в тексте строки.
- Коды: `403` — токен только для чтения; `404` — чужая выдача (её вам не видно);
  `409` — выдача уже отозвана.

**Выдать доступ по токену нельзя — `403`.** Выдача создаётся владельцем в GUI. Причина не
в осторожности: выдача пишет ИМЯ ключа в чужое пространство имён. Значение приезжает
получателю в общий ответ `GET /api/secrets`, его собственный ключ выигрывает, но **новое**
имя подмешивается молча — а рецепты выше учат раскладывать полученное в переменные
окружения CI. Токен одной комнаты означал бы подстановку произвольной переменной в чужую
сборку, и платит за это не тот, кто её совершает.

Машинный путь вернётся **двусторонним**: источник предлагает выдачу, получатель принимает
её своим токеном. Обе стороны машины, владелец по-прежнему не нужен — но согласие
получателя появляется. Нужна выдача сейчас — через владельца или brain.
