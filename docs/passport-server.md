...
- **Машинного grant'а нет** — вырезан адверсариальной проверкой: цель выбирал вызывающий,
  согласия цели не требовалось, а в связке с шагом доставки это RCE на чужом проде.
  Выдача — операция владельца данных под 2FA.
  ...
  **Условие возврата** — двусторонность: источник предлагает, получатель принимает своим
  токеном. Обе стороны машины, значит «ключи ходят без рук владельца» (D-061) выполняется,
  а «согласия цели не требуется» перестаёт быть верным. Чтение (`GET`) и отзыв (`DELETE`)
  своих выдач по токену остались: они не подкладывают ничего никому.
