...
Общий смысл списка: каждая проверка отвечает на вопрос «**тот ли** это токен», а не «наш ли
он». Подпись доказывает происхождение, но не назначение — грабля
`../brain_matrica/cross-project-ideas/GOTCHAS.md` → **G305**, и она наша собственная: в
многошаговом входе КАРМАНа этапный токен (до 2FA) и сессионный были подписаны одним ключом
с одним `uid`, поэтому значение cookie этапа, переложенное в cookie сессии, открывало вход
без второго фактора. Дефект пережил код-ревью внедрения 2FA, внедрение ЕСА и год
эксплуатации; нашёл его скептик, которому поручили **опровергнуть** утверждение о
безопасности. Регрессия — `lib/auth/jwt.test.ts`.

Отсюда правило для любого нового вида токена в этом контуре: вид несёт метку, и проверяющая
функция сверяет именно её. **Минутная проверка при добавлении токена:** найти все места, где
JWT подписывается тем же ключом, и для каждой проверяющей функции спросить, отличит ли она
токен другого назначения. Один вид — граблей нет; два и более — смотреть.
...
### Штатный путь: раздел `/secrets/identities`

Кнопка «Реестр личностей» в шапке `/secrets`. Форма спрашивает издателя, метку, номер
репозитория, комнату и право записи; отзыв — из строки, каскадом, с показом числа
сессий, которые погаснут. Обе операции пишут аудит комнаты (`identity_created`,
`identity_revoked`, актор `owner:N`).

Колонка **«Последний вход»** — не украшение: отказ «личность вне реестра» ложится строкой
без комнаты, и в аудите самой комнаты его не видно. Личность, которая числится заведённой
и ни разу не входила, — почти всегда неверный номер репозитория.

Сменить комнату или право у заведённой личности нельзя: `can_write` копируется в токен при
выдаче, а `identity_value` — это и есть личность. Штатный путь — отзыв и заведение заново;
частичный уникальный индекс это допускает специально.

### Runbook владельца: завести личность (аварийный путь, SQL)

Ниже — то же самое руками: пригодится, если интерфейс недоступен. Идентификатор берётся
у GitHub, комната — **из живого `/secrets`**, а не из документации.
...
- **Одностороннего машинного grant'а нет** — вырезан адверсариальной проверкой: цель
  выбирал вызывающий, согласия цели не требовалось, а в связке с шагом доставки это RCE на
  чужом проде.

  **Границу однажды сняли и вернули обратно 2026-09-03.** По мандату был выкачен
  `POST /api/secrets/grants` под токеном комнаты-источника; довод «это та же граница, что у
  `POST /api/secrets`» оказался неверен. Запись в свою комнату остаётся в своей комнате, а
  выдача пишет ИМЯ в чужое пространство имён: значение приезжает получателю в общий ответ
  `GET /api/secrets`, собственный ключ получателя выигрывает, но новое имя подмешивается
  молча — а клиентский рецепт учит раскладывать полученное в переменные окружения CI.
  Токен одной комнаты давал подстановку произвольной переменной в чужую сборку.
  Воспользоваться никто не успел (аудит: все выдачи `owner:1`).

  **Возвращён 2026-09-04 двусторонним** (мандат brain 03.09, миграция `0014`): источник
  предлагает своим пишущим токеном (`POST /api/secrets/grants` → строка `pending`),
  получатель принимает своим пишущим токеном (`POST …/grants/<id>/accept`). До принятия
  предложение не входит в `GET /api/secrets` получателя и имя у него не занимает
  (уникальный индекс — только по принятым); гейт «не принято и не отозвано» стоит в
  `WHERE` самого `UPDATE`. Обе стороны машины, значит «ключи ходят без рук владельца»
  (D-061) выполняется, а «согласия цели не требуется» перестаёт быть верным. Отзыв
  (`DELETE`) — с любой стороны. Что остаётся границей: **односторонняя** форма — предложение,
  которое активно без accept, или accept читающим токеном.
- **Комнату паспорт не заводит.** Неизвестная личность → `403`, и точка.
- **Реестр ведётся из интерфейса** — раздел `/secrets/identities` (веха 2 волны 2):
  список с числом живых сессий и последним входом, заведение формой, отзыв каскадом.
  SQL-runbook'и выше остаются как аварийный путь и как описание того, что делает
  интерфейс. **Заведение — только владелец vault** (superuser): пространство
  `(issuer_id, identity_value)` глобально, комнату заводит любой вошедший, а вошедшим
  становится любой прошедший вход через ЕСА, — без этого гейта посторонний
  зарегистрировал бы `repository_id` чужого репозитория на свою комнату. Отзыв —
  по владению комнатой.
- **Реестра личностей ЕСА (людей) в разделе нет.** Отвязка там — физический `DELETE`
  без `revoked_at`, таблицы `auth_oidc_identity` нет в списке бэкапа, а очевидный гейт
  «оставить человеку путь входа» не пишется одной строкой: пользователю, заведённому
  через ЕСА, кладётся структурно валидный хэш заведомо непригодного пароля, поэтому
  проверка «пароль не задан» на нём всегда ложна. Это отдельная веха.
