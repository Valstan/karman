...
_Нет — последняя задача закрыта._ Сессия 12.09 сделала пять шагов, все на проде:

| Что | PR |
|---|---|
| Карта проектов `/map` по мандату brain 11.09; данные — JSON на боксе вне git (D-038), обновление по письму = копирование файла; ответ brain отправлен | #141 |
| Концепция: KARMAN — личный органайзер «всё своё при себе», не «учёт кредитов» (подвал, `<title>`, вход, README, AGENTS, ARCHITECTURE) | #142 |
| «Чей документ» (миграция 0017), фильтр/колонка по людям, шаблоны под реальные графы; категории шаблонов починены под прод (падали в «Прочее») | #143 |
| Загрузка документов семьи: 59 документов, 81 скан, 323 поля в аккаунт `admin` (владелец, жена, 5 детей, брат, «Семья»); прод-баг `MEDIA_ROOT` найден и закрыт | #144 (доки) |
| Новый интерфейс: во всю ширину, градиентный фон, тёмные кнопки с жёлтыми буквами, инверсия активного раздела, перенос в ячейках; в «Документах» карточки людей и выборка «люди × виды» → поделиться / zip / Word | #145 |

## Следующий шаг

Открытой нитки нет. Первое — **смоук владельца глазами** (локальной БД нет, экраны за
логином никто не видел): `/documents` — карточки людей, фильтр, «Собрать выборку» → .zip со
сканами; таблицы кредитов/платежей на его экране; тёмная тема. Три пометки «уточнить по
скану» в данных (свидетельство о рождении Ольги, диплом колледжа, приписное Даниила).
Дальше кандидаты по приоритету — `docs/PENDING_FOLLOWUPS.md`: R1 (тесты инвариантов доступа,
Q4), гейт таблиц бэкапа, G321, ответы brain.
...
- **Почта brain:** новых входящих на 12.09 нет; отправлен отчёт по карте
  (`mailbox/to-brain/2026-09-12-ecosystem-map-…`) и идея про `MEDIA_ROOT`.
