...
1. Рекомендация Мозга 12.09: строка `project_deleted` в аудит секретов **до** каскадного
   `DELETE`, в той же транзакции (`project_id = NULL`, как у `provision_denied`); строкой
   Мозгу, когда на проде.
2. Дайджест Мозга 12.09 (ack none): наше — #312 (`npm audit` по расписанию, не только в PR),
   G340 (проверить, что метрик/Вебвизора на страницах после входа нет).
3. Смоук владельца по #145 (документы, новый интерфейс) — всё ещё не сделан глазами.
